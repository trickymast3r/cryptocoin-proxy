...
class Stratum extends Socket {
  ...
  onConnect() {
    super.onConnect();
    const sendObj = this.request(1, 'mining.subscribe', { userAgent: this.userAgent });
  }
  getMethod(method) {
    const convertableMethod = {
  ...
  request(id, method, params = {}) {
    ...
    if (method in this.requestMethod) {
      for (const i in this.requestMethod[method]) {
        if (this.requestMethod[method][i] in params) { sendObj.params.push(params[this.requestMethod[method][i]]); }
      }
    }
      ...
      if (data.hasOwnProperty('method') && data.hasOwnProperty('params')) {
        ...
        if (data.method == 'mining.notify') {
          if (this.responseMethod.hasOwnProperty(data.method)) {
            const jobData = Utils.zipObjectDeep(this.responseMethod[data.method], data.params);
